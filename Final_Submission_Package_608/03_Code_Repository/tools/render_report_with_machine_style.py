"""Render the final report using the bundled report visual style."""
...
def extract_reference_css() -> str:
    ...
    css += """
        ...
        figure img { max-height: 560px; object-fit: contain; }
        @media print {
...
def pandoc_executable() -> str:
    ...
    if pandoc:
        return pandoc
    raise FileNotFoundError("pandoc is required to render the report HTML/PDF.")
...
<div class="report-note">
    ...
    Generated with the bundled report visual style.
</div>
</div>
</body>
</html>
...
def chrome_executable() -> str | None:
    ...
    for app_path in app_paths:
        executable = Path(app_path) / "Contents" / "MacOS" / "Google Chrome"
...
def render_pdf() -> None:
    ...
    if chrome:
        report_pdf = REPORT_PDF.relative_to(ROOT).as_posix()
        cmd = [
            chrome,
            "--headless=new",
            "--disable-gpu",
    ...
    cmd = [
        ...
        "-o",
        REPORT_PDF.relative_to(ROOT).as_posix(),
        "--resource-path=.:report:outputs/report_ready",
        "--pdf-engine=xelatex",
        "-V",
...
def render_tex_snapshot() -> None:
    cmd = [
        ...
        REPORT_MD.relative_to(ROOT).as_posix(),
        "-o",
        REPORT_TEX.relative_to(ROOT).as_posix(),
        "--resource-path=.:report:outputs/report_ready",
        "--standalone",
    ]
    subprocess.run(cmd, cwd=ROOT, check=True)
...
def main() -> None:
    ...
    render_pdf()
    render_tex_snapshot()
    print(f"Wrote {REPORT_HTML.relative_to(ROOT).as_posix()}")
    print(f"Wrote {REPORT_PDF.relative_to(ROOT).as_posix()}")
