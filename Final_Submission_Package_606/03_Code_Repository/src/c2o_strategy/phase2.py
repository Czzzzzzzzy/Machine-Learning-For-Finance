...
def load_or_score_phase2_panel(
    ...
    if spec.training_window == DEFAULT_TRAINING_WINDOW and spec.alpha_learning_method == DEFAULT_ALPHA_METHOD:
        return load_or_score_target_panel(panel, rank_columns, config, "vol_scaled")

    path = phase2_cache_path(config, spec.scored_key)
    if path.exists():
        print(f"Loading cached Phase 2 scored panel {spec.scored_key} from {path}...", flush=True)
    ...
    print(f"Caching Phase 2 scored panel {spec.scored_key} to {path}...", flush=True)
