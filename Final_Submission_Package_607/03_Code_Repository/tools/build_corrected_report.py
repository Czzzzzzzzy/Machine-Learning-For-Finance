...
ICLOUD_DIR = (
    Path.home() / "Library" / "Mobile Documents" / "com~apple~CloudDocs" / "IC_iCloud"
)
...
def make_capacity_plot(capacity: pd.DataFrame) -> Path:
    ...
    ax.bar(labels, capacity["average_gross_exposure_used"] * 100.0, color=["#2563eb", "#16a34a", "#dc2626"])
    ax.set_ylim(0, 110)
...
The report does not use direct prime-broker borrow-fee, locate-rate, or securities-lending data. That is a limitation. The mitigation is explicit conservative A/B/C borrow charging.
...
The original baseline archive remains in `outputs/baseline_archive/`. The previous 0.811 champion remains in `outputs/current_champion_archive/` and `outputs/previous_champion_archive_after_phase2/`.

## 10. Limitations

The main limitations are:

- No direct external borrow-fee or locate-rate validation.
...
```bash
PYTHONPATH=src /opt/anaconda3/bin/python3 -m c2o_strategy.final_strategy --data-dir data --output-dir outputs --cutoff 2024-12-31 --development-cutoff 2024-12-31 --mode all
PYTHONPATH=src /opt/anaconda3/bin/python3 -m pytest -q
```

The latest recorded run passed with 13 tests. The optional LightGBM diagnostic was run with LightGBM 4.6.0 and wrote `outputs/report_ready/lightgbm_diagnostic.csv`. The model-family robustness screen wrote `outputs/model_family_robustness_summary.csv` and `outputs/report_ready/model_family_robustness_memo.md`.
...
def main() -> None:
    ...
    if ICLOUD_DIR.exists():
        shutil.copy2(corrected_md_path, ICLOUD_DIR / "final_report_corrected.md")
